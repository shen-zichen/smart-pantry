...
/**
 * Manages the pantry inventory and acts as the Subject in the Observer Pattern. After any consume
 * or restock operation, it checks for alert conditions and notifies all registered observers.
 *
 * <p>This is the central hub for inventory operations. Controllers interact with the pantry through
 * this class, never directly with PantryItem lists.
 */
@Component
public class InventoryManager implements IInventorySubject {
  ...
  /**
   * Checks all pantry items for alert conditions and notifies observers. Called automatically after
   * consume/restock, but can also be called manually (e.g., daily expiration check).
   */
  @Override
  ...
  /**
   * Consumes a quantity of a pantry item and triggers observer notifications. This is the key
   * method — it's where the Observer Pattern fires.
   *
  ...
  public void setExpirationWarningDays(int days) {
    if (days < 0) {
      throw new IllegalArgumentException("Expiration warning days cannot be negative: " + days);
    }
